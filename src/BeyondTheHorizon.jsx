 ...
 *   public/assets/destination-sfr.jpg      San Francisco hub (-sfr, not -sfo)
...
const HUBS = [
  ...
  },
  {
    ...
    // NOTE: filename is -sfr, not -sfo. Every other asset follows the airport
    // code; this one deliberately doesn't. Keep them in sync if you rename.
    img: "./assets/destination-sfr.jpg",
    hook:
...
const METRICS = [
  ...
  { icon: TrendingUp, value: "17.4%", label: "Avg. engagement rate", sub: "Compared to 2.8% industry average" },
...
export default function BeyondTheHorizon() {
  ...
  // Films share their chapter's artwork — one source of truth for asset paths.
  const filmChapter = CHAPTERS.find((c) => c.code === film.code);
        ...
        <Section id="films">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              transition={{ duration: 0.5, ease: EASE }}
            >
              {/* Film header — destination artwork paired with the logline */}
              <div className="mt-10 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-xl">
                <div className="grid md:grid-cols-[0.95fr_1.05fr]">
                  <div className="relative h-60 w-full overflow-hidden md:h-auto md:min-h-[330px]">
                    <Media
                      src={filmChapter?.img}
                      alt={`${film.title}, ${filmChapter?.country} — ${film.route}`}
                      className="h-full w-full object-cover"
                    />
                    {/* Scrim blends into the card: upward on mobile (image on
                        top), rightward on desktop (image beside the text). */}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0A0D14] via-[#0A0D14]/15 to-transparent md:bg-gradient-to-r md:from-transparent md:via-transparent md:to-[#0A0D14]/80" />
                    <span className="bth-mono absolute left-6 top-6 rounded-full border border-[#00A3E0]/40 bg-[#05070B]/70 px-3 py-1 text-[10px] uppercase tracking-[0.26em] text-[#00A3E0] backdrop-blur-md">
                      {film.code}
                    </span>
                  </div>
                  ...
                  <div className="p-8 md:p-10">
                    ...
                    <p className="bth-mono mt-2 text-[10px] uppercase tracking-[0.2em] text-[#94A3B8]/70">
                      {filmChapter?.chapter} · {filmChapter?.city},{" "}
                      {filmChapter?.country}
                    </p>
                    <p className="mt-5 max-w-2xl text-sm leading-relaxed text-[#94A3B8] md:text-base">
                      {film.logline}
                    </p>
                    <div className="mt-7">
                      <StatusLine />
